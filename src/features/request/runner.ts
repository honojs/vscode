...
import { buildBundledHonoRequestArgv, type RequestInvocationInput } from "./argvBuilder";
...
export async function runRequestOnce({ args, output, extensionPath }: RunnerDeps): Promise<void> {
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: `Hono request: ${args.path}`, cancellable: false },
    async () =>
      new Promise<void>((resolve) => {
        ...
        child.on("close", (code, signal) => {
          ...
          if (code === 0) {
            ...
            output.show(true);
            void vscode.window.showErrorMessage(`Hono request failed (exit ${code ?? "null"}). See Output: Hono`);
          }
          resolve();
        });
      })
...
export async function runRequestWatchInTerminal({ args, output, extensionPath }: RunnerDeps): Promise<void> {
  const cwd = getWorkspaceCwdForUri(args.uri);
  if (!cwd) {
    void vscode.window.showErrorMessage("Workspace folder not found.");
    return;
...
export async function runRequestDebug({ args, output, extensionPath }: RunnerDeps): Promise<void> {
  ...
  output.appendLine("");
  output.appendLine(`[debug] launching with inspector: ${cfg.nodePath} --inspect-brk=0 ${entry} request -P ${resolved.path}`);
  output.show(true);
...
async function resolveInvocationInput(args: RequestLensCommandArgs): Promise<RequestInvocationInput | undefined> {
...
async function promptFormBodyFromTypes(fields: FormFieldSpec[]): Promise<{ data?: string; headers?: string[] } | undefined> {
